...
(function () {
    ...
    function kioskInit() {
        var userName = $app.userName(),
            userAvatar = $('<span class="app-avatar"/>').attr('data-title', userName).appendTo(_kiosk),
            currentNode = Web.Menu.currentNode,
            copyright = uiAutomationSettings('kiosk.copyright'),
            logo,
            buttonAnchor, buttonRight;

        logo = $('<span class="app-logo"/>').appendTo(_kiosk).html(_touch.appName());
        if (currentNode)
            _touch.icon('material-icon-chevron-right', $('<span class="app-page-title"/>').appendTo(logo.attr('data-title', currentNode.description)).text(currentNode.title));

        $('<span class="app-btn-menu ui-btn ui-btn-icon-notext ui-btn-left ui-icon-bars"/>').appendTo(_kiosk).attr('data-title', resourcesMobile.Menu);
        buttonAnchor = $('<span class="app-btn-context ui-btn ui-btn-icon-notext ui-btn-left ui-icon-bars"/>').appendTo(_kiosk).attr('data-title', resourcesMobile.More);


        _app.AccountManager.avatar(userName, $('<i class="app-icon-avatar"/>').text(_touch.initials(_app.userName())).appendTo(userAvatar));
        if (_app.userName())
            buttonAnchor = userAvatar;
        else {
            userAvatar.hide();
            if (!$body.data('public'))
                $('<span class="app-login ui-btn"></span>').appendTo(_kiosk).attr('data-title', Web.MembershipResources.Bar.LoginLink);
        }
        ...
        for (var i = 1; i < 3; i++) {
            buttonRight += 50;
            $('<span class="ui-btn ui-btn-icon-notext ui-btn-left app-btn app-btn' + i + '"><i class="app-icon material-icon"></i></span>').css('right', buttonRight).appendTo(_kiosk).hide();
        }
    }
    ...
    _app.display = function (options) {
        ...
        function flowChanged() {
            var saveScrollTop = wrapper.scrollTop(),
                ...
                centerRow;
            //if (restoreScrollTop) {
            //    centerRow = $(document.elementFromPoint(wrapper.offset().left + wrapper.width() / 2, wrapper.offset().top + wrapper.height() / 2)).closest('.app-row,.app-hero,-app-row-container').last();
            //    if (currentRow.length) {
            //        // TODO: find the row in the new layout and scroll to it to provide a better tracking upon resize.
            //    }
            //}
        ...
        if (Array.isArray(options))
            $document.one('displayflow.app', function (e) {
                ...
                options.forEach(function (displayObj) {
                    ...
                    if (content != null) {
                        ...
                        if (content.match(/^(#|\.)[\w-]+/)) {
                            ...
                            if (html == null) {
                                html = [];
                                page.find(content).remove().each(function () {
                ...
                wrapper.data('displayFlow', flowDef).find('.app-page-content').addClass('app-display-flow')
            });
        else {
            var wrapper = $(options),
                pageContent,
    ...
    _app.display.as = {
        code: {
            render: function (config, container, content) {
                if (typeof config == 'string')
                    config = { type: config };
                container.addClass('app-render-as-code');
                var headers = container.find('h1,h2,h3,h4,h5,h6').remove(),
                    code = container.find('pre code').first(),
                    editor,
                    mode = config.type;
                if (mode)
        ...
        function heroComplete() {
            if (currentHeroId) {
                var hero = currentHero.parent(),
                    ...
                    isFullHeight = heroHeight <= displayHeight,
                ...
                if (iconDef !== false) {
                    ...
                    if (typeof iconDef == 'string')
                        $('<i class="material-icon"/>').text(iconDef).insertBefore(currentHero).css({ 'transform': 'scale(' + iconSize / 24 + ')', right: iconSize / 2 });
                    else if (icon.length)
                ...
                if (isFullHeight && currentHeroDef.sticky) {
                    ...
                    hero.appendTo(stickyHeroContainer).addClass('app-spacing-none app-hero-sticky')
                        .find('.app-hero-content').appendTo(div('app-hero')
            ...
            if (config._newRow) {
                ...
                if (heroDef) {
                    ...
                    if (heroId === true || heroId !== currentHeroId) {
                        ...
                        currentHero = div('app-hero-content').appendTo(div('app-hero')
                ...
                if (isJumbo)
                    (hasFluidBackground && !heroDef ? currentRowParent : currentRow).addClass('app-jumbo');
                if (background)
                    (hasFluidBackground && !heroDef ? currentRowParent : currentRow).addClass('app-has-background').toggleClass('app-background-accent', background.accent === true);
            }
            if (config._newCol) {
                ...
                if (!config._newRow && background)
                    currentContainer.addClass('app-has-background').toggleClass('app-background-accent', background.accent === true);
            }
            ...
            if (inDataView) {
                ...
                configIterator(config, function (displayObj, name) {
                    ...
                    items.push({
                        html: htmlContent ? htmlContent :
                            ...
                            (isFieldOrExpression ? '<span class="app-label"/><span class="app-value"/>' : '<span class="app-button app-feedback"><span class="app-text"/></span>') +
                            '</div>',
                        obj: displayObj
                    });
                ...
                if (config._propCount) {
                    ...
                    if (configIsList(config)) {
                        ...
                        if (totalColumns) {
                            ...
                            gridObjects.forEach(function (displayObj) {
                                ...
                                $('<span class="app-text"/>').appendTo(setWidth($('<span class="app-column-header"/>'), displayObj._width).appendTo(headerRow)
                    ...
                    if (config.isContent) {
                        ...
                        contentElement = $(content).appendTo(container);
                        if (flowAs) {
                            ...
                            for (rendererName in flowAs) {
                                ...
                                if (rendererImplementation)
                                    contentElement = rendererImplementation.render(flowAs[rendererName], container, content);
                            }
                        }
                    }
                    else {
    ...
    function alterDisplayObject(config) {
        var alter = config.alter,
            ...
            privatePropertyExceptions = ['_lastResult'],
            privateProperties = [];
    ...
    function displayItemsInContainer(config, container) {
        ...
        items.forEach(function (item) {
            ...
            if (isGridColumn || displayObj._label === false)
                html = html.replace('<span class="app-label"/>', '');
        ...
        configIterator(config, function (displayObj, name) {
            ...
            if ((field || expression) && data) {
                ...
                if (valueIsNull)
                    ...
                    text = _touch.toSmartDate(value, new Date());
                    itemValue.attr({
            ...
            genericPropItem.css({ backgroundColor: toPropValue(displayObj.colorBackground, data), color: toPropValue(displayObj.color, data) })
            if (displayObj.when)
    ...
    function configBindDataView(config, container, result) {
        ...
        if (pageSize < totalRowCount) {
            ...
            _touch.icon('material-icon-navigate-before', $('<span class="app-btn app-feedback app-prev"/>').appendTo(footer).attr('data-title', resourcesPager.Previous).toggleClass('app-disabled', !pageIndex));
            _touch.icon('material-icon-navigate-next', $('<span class="app-btn app-feedback app-next"/>').appendTo(footer).attr('data-title', resourcesPager.Next).toggleClass('app-disabled', pageIndex === Math.ceil(totalRowCount / pageSize) - 1));
            $('<span class="app-info"/>').appendTo(footer).html(String.format(resourcesPager.ShowingItems, pageIndex * pageSize + 1, Math.min(totalRowCount, (pageIndex + 1) * pageSize), totalRowCount));
        }
    ...
    function execute(options) {
        ...
        if (execState.index < ifttt.length) {
            ...
            if (activity.background || id && activity.id === id) {
                ...
                $.when(appDeferred)
                    ...
                    .fail(next);
                try {
                    activity.execute(app, execState.context).done(iftttDone).fail(iftttFail);
                }
                catch (ex) {
                    iftttDone(ex);
                }
